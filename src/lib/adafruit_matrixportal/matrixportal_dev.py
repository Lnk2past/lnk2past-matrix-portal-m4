...
class MatrixPortal(PortalBase):
    ...
    def scroll(self, scrolling_indices=None):
        ...
        if scrolling_indices is None:
            if self._scrolling_index is None:  # Not initialized yet
                return
            scrolling_indices = [self._scrolling_index]

        for scrolling_index in scrolling_indices:
            self._text[scrolling_index]["label"].x -= 1
            line_width = (
                self._text[scrolling_index]["label"].bounding_box[2]
                * self._text[scrolling_index]["scale"]
            )
        
            if self._text[scrolling_index]["label"].x < -line_width:
                self._text[scrolling_index]["label"].x = self.graphics.display.width
